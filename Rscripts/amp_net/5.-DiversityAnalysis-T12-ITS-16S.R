...
#Tabla para los datos del TDA
#write.table(alpha,
#            file=paste(getwd(),paste("index",data,"_tmap","txt",sep = "."),sep = "/"),
...
alpha=cbind(alpha, met[,c( "compartment","sampling", "ID_seq_ITS", "treatment","site")])
#alpha=cbind(alpha, met[,c("sampling", "ID_seq_16s")])
...
tapply(alpha$richness, alpha$sampling,shapiro.test)
tapply(alpha$shannon, alpha$sampling,shapiro.test)

#tapply(alpha$richness, alpha$treatment,shapiro.test)
#tapply(alpha$shannon, alpha$treatment,shapiro.test)
...
mean.r=summarySE(data = alpha, measurevar = "richness", groupvars = c("sampling","compartment","treatment"))
#mean.r=summarySE(data = alpha, measurevar = "richness", groupvars = c("sampling","compartment"))
mean.r$sampling=factor(mean.r$sampling, level = pc)
mean.s=summarySE(data = alpha, measurevar = "shannon", groupvars = c("sampling","compartment","treatment"))
#mean.s=summarySE(data = alpha, measurevar = "shannon", groupvars = c("sampling"))
mean.s$sampling=factor(mean.s$sampling, level = pc)

#Analisis estadisticos diversidad  
#RICHENESS BY 
pdf(paste(amplicon,"deleteya2",file = "sampling-richness.pdf"), colormodel = "cmyk", width = 15, height = 9, compress = F)
p=ggplot(data=mean.r, aes(x=sampling, y=richness, fill=sampling))+
  ...
  scale_fill_manual(values = c("T0"="#E7E1EF","T6"="#E7E1EF","T12M"="#D4B9DA","T12P"="#C994C7"))+
  #facet_wrap(~ compartment~treatment, nrow = 4)+
...
pdf(paste(amplicon,"rizo.root.leaf.filo",file= "sampling.shannon.pdf"), colormodel = "cmyk", width = 15, height =9, compress = F)
...
#plot
pdf(paste(amplicon,"T12 soil.root",file = "NMDS.comparment.pdf"), colormodel = "srgb", width = 11, height = 8.5, compress = F)
n=ggplot(data=scaling3, aes(x=MDS1, y=MDS2, colour= sampling, shape= compartment))+
  geom_hline(yintercept = 0, linetype=2)+
  geom_vline(xintercept = 0,linetype=2)+
  geom_point(size=7)+
  #scale_colour_manual(values = my_pal(9)[2:5])+
  scale_colour_manual(values = c("#2A2A4E","#FA1F6F","#FF8226","#059986","#00A5BB"))+
...
m=ggplot(data=scaling3, aes(x=MDS1, y=MDS2, colour=treatment, shape=sampling))+
  geom_hline(yintercept = 0, linetype=2)+
  geom_vline(xintercept = 0,linetype=2)+
  geom_point(size=9)+
  #scale_colour_manual(values = my_pal(9)[2:5])+
...
#adonis2(scaling~sampling*treatment, data = met, permutations = 1000)
adonis2(scaling~treatment*compartment, data = met, permutations = 1000)

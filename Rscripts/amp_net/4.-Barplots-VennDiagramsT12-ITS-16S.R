...
my_pal=colorRampPalette(c("#32FF8F","#009845","#FFD100","#FF8B00","#F48FB1","#F50087","#C2185B","grey90","dimgrey"))
pc=c("soil","rhizosphere", "root.endosphere", "leaf.endosphere", "phyllosphere", "T0", "T6", "T12M", "T12P", "MOCK", "EV","CAMP","PFC","PFCS")
#Upload data R Base
if (amplicon == "ITS2"){
  ...
  otu.t0 = otu.t %>% group_by(compartment,treatment,sampling) %>% summarise(absab = sum(abs))
  otu.t1 = otu.t %>% group_by(otu.t[,t],compartment,treatment,sampling) %>% summarise(absab = sum(abs))
  names(otu.t1)[1]=c("rank")
  sum(paste(otu.t1$compartment,otu.t1$treatment,otu.t1$sampling,sep = ".")==paste(otu.t0$compartment,otu.t0$treatment,otu.t0$sampling,sep = "."))==dim(otu.t1)[1]
  otu.t1$relab=otu.t1$absab/otu.t0$absab*100
  #Remove low abundant taxa
  orden = otu.t1 %>% group_by(rank) %>% dplyr::summarise(relab = mean(relab)) 
  orden1 = otu.t1 %>% group_by(rank, treatment) %>% dplyr::summarise(relab = mean(relab)) 
  #Check how many groups you can select for plots, modify relab
  #order
  low = orden[orden$relab < 1.361176868, "rank"]
  #phylum
  ...
  otu.t1$sampling=factor(otu.t1$sampling, level = pc)
  otu.t1$compartment=factor(otu.t1$compartment, level = pc)
  otu.t1$treatment=factor(otu.t1$treatment, level = pc)
  
  ##########################################################
  #subset only glomeromycota Nestor Analysis comment line when are doing thesis analysis
  ####Total reads by PLANT SPECIE AND PLANT COMPARTMENT
  t=c("domain","phylum","class","order","family","genus","otu.id")[4]
  ##Sumarise data
  set.seed(23171341)
  ...
  otu.t[is.na(otu.t[,t]),t]="Unclassified"
  for (i in met$ID_seq_ITS){
    otu.t[otu.t$sample == i, c("compartment","sampling","ID_seq_ITS")] = met[i,c("compartment","sampling","ID_seq_ITS")]
  }
  otu.t0 = otu.t %>% group_by(compartment,sampling) %>% summarise(absab = sum(abs))
  #subset only glomeromycota
  otu.t= subset(otu.t, tax$phylum =="g:Kellermania", 
                select= c("domain","phylum","class","order","family","genus","otu.id","abs", "compartment","sampling"))
  otu.t1 = otu.t %>% group_by(otu.t[,t],compartment,sampling) %>% summarise(absab = sum(abs))
  names(otu.t1)[1]=c("rank")
  sum(paste(otu.t1$compartment,otu.t1$sampling,sep = ".")==paste(otu.t0$compartment,otu.t0$sampling,sep = "."))==dim(otu.t1)[1]
  otu.t1$relab=otu.t1$absab/otu.t0$absab*100
  #Remove low abundant taxa
  orden = otu.t1 %>% group_by(rank) %>% summarise(relab = mean(relab))
  orden1 = otu.t1 %>% group_by(rank, compartment) %>% summarise(relab = mean(relab)) 
  low = orden[orden$relab < 0, "rank"]
  otu.t1[otu.t1$rank %in% low$rank, "rank" ] = "other"
  n=dim(orden)[1]-length(low$rank)+1 ; print(c(n,"taxa")) 
  #Reorder taxa if needed
  ...
  #Checklist
  colSums(otu.s)
  #tax <- apply(tax, 1, function(row) paste(row, collapse = "_")) #abel tmap
  #tax <- as.data.frame(tax) #abel tmap
  #sum(rownames(otu.s)==row.names(tax))==dim(otu.s)[1]
  otu.s=as.data.frame(cbind(otu.s,tax))
  #write.table(otu.s, 
  #            file=paste(getwd(),paste("otu.table",data,"_tmap_reads","txt",sep = "."),sep = "/"),
  #            quote = F, col.names = T, row.names = T, sep = "\t") #abel
  otu.t=gather(data=otu.s, key = "sample", value = "abs", colnames(otu))
  otu.t[is.na(otu.t[,t]),t]="Unclassified"
  for (i in met$ID_seq_16s){
    otu.t[otu.t$sample == i, c("compartment","treatment","replica","site", "sampling", "ID_seq_16s")] = met[i,c( "compartment","treatment","replica","site", "sampling", "ID_seq_16s")]
  }
  ...
  #Check how many groups you can select for plots, modify relab
  #Order
  low = orden[orden$relab < 1.19488843, "rank"]
  #Phylum or OTU
  #low = orden[orden$relab < 0, "rank"]
  #Genus
...
pdf(paste(amplicon,"glomer", t, file = "RA BY sampling.pdf"), colormodel = "cmyk", width = 15, height = 11, compress = F)
p=ggplot(data=otu.t1, aes(y=relab, x=sampling, fill=rank))+
  geom_bar(stat="identity",width = 0.95)+
  scale_fill_manual(values = my_pal(n))+
  labs(x = "sampling",y = "relative abundance (%)")+
  facet_grid(~compartment, scales = "free", space = "free_x")+
  guides(fill=guide_legend(title = "class", ncol = 1, keyheight = 0.7, default.unit="cm",
                           label.theme = element_text(size= 18, face = "plain", family = "sans")))+tema
p
dev.off()
...
p1=ggplot(data=otu.t1, aes(y=relab, x=plant.compartment, fill=rank))+
  ...
  scale_fill_manual(values = my_pal(n))+
  labs(x = "Plant species",y = "relative abundance (%)")+
  facet_grid(~compartment, scales = "free", space = "free_x")+tema+
  theme(legend.position = "none")
p1
dev.off()
